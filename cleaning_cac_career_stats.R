...
raw_stats <- read_csv("~/Desktop/all_cac_players_ever_stats.csv")
...
a_draft_stats <- get_tidy_total_stats(raw_stats) %>%
  filter(level == "A Draft 4v4") %>% get_per_game_averages()

a_draft_all_totals <- get_tidy_total_stats(raw_stats) %>%
  filter(level == "A Draft 4v4") %>% get_per_game_averages()
